...
cs_project = os.path.dirname(os.path.dirname(dirname))
...
# load labels
labels = torch.tensor(np.loadtxt(os.path.join(cs_project, "Data/train.txt"))[:, None], dtype = torch.float)
n = labels.shape[0]
    ...
    for batch_num, batch in enumerate(tqdm(np.array(train_id).reshape(-1, batch_size))):
        for i, im in enumerate(batch):
            for c in np.arange(rgb_channels):
                inputs[i, c, :, :] = cv2.imread(cs_project + "/Data/color_images/frame_" + str(im) + ".jpg")[:, :, c]
